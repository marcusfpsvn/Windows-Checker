...
$host.ui.RawUI.WindowTitle = "MarcusFPS Windows Checker 3.6"

# =========================
# TỰ ĐỘNG CĂN CHỈNH KÍCH THƯỚC CỬA SỔ (ÉP BUỘC BẰNG MODE)
# =========================
try {
    # 1. Dùng lệnh hệ thống ép cứng số cột (cols) và số dòng (lines)
    mode con cols=115 lines=55
    
    # 2. Mở rộng bộ nhớ đệm (Buffer) để nếu có dài hơn thì vẫn cuộn lên xem được, không bị xóa mất chữ
    $pswindow = $host.ui.RawUI
    $pswindow.BufferSize = New-Object System.Management.Automation.Host.Size(115, 300)
} catch {}
...
function Show-Main {
    ...
    Write-Host "    Phát triển bởi MarcusFPS - Phiên bản 3.6"
    Write-Host "        Cập nhật: 36/07/2026 - https://madebythanh.art/"
    Write-Host "=================================================="
...
function Show-Check {
    ...
    Write-Host ""
    # =========================================================
    # BỔ SUNG: LÀM MỚI THÔNG TIN PHIÊN BẢN WINDOWS TỪ REGISTRY
    # =========================================================
    try {
        $script:OS_NAME = (Get-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion" -ErrorAction SilentlyContinue).ProductName
        $TEMP_BUILD = (Get-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion" -ErrorAction SilentlyContinue).CurrentBuild
        if ([int]$TEMP_BUILD -ge 22000) {
            $script:OS_NAME = $script:OS_NAME -replace "Windows 10", "Windows 11"
        }
    } catch {}
    # =========================================================
    
    # KHỞI TẠO BẢNG ĐIỂM (Thêm reset HWID_WARN vào đây)
    $script:HWID_WARN = "0"
    ...
    if ($hwidKeys -contains $script:PK) {
        ...
        if ($script:BIOS_KEY -eq "KHONG_TIM_THAY") {
            ...
            if ($script:ACT -eq "CHƯA KÍCH HOẠT") {
                $script:scan_logic = $true
                $script:msg_logic = "Sử dụng Key chung để nâng cấp/chuyển phiên bản (Chưa kích hoạt)."
            } else {
                # SỬA LỖI TẠI ĐÂY: Đã kích hoạt bản cao hơn. Có thể là Mua thật hoặc MAS HWID.
                # Chuyển thành True để không báo đỏ, nhường quyền phán quyết cho thu ngân/người check.
                $script:scan_logic = $true 
                $script:msg_logic = "Dùng Giấy phép số bản cao hơn Key BIOS (Cần xác minh)."
            }
        }
    }
...
function Show-Legal {
   ...
   } elseif ($script:HWID_WARN -eq "1") { # Đã bỏ điều kiện KHONG_TIM_THAY
        [Console]::ForegroundColor = "Yellow"
...
Write-Host " liên kết phần cứng." -ForegroundColor White
   ...
   # XỬ LÝ THÔNG BÁO BIOS KEY
    if ($script:BIOS_KEY -ne "KHONG_TIM_THAY") {
        [Console]::ForegroundColor = "Cyan"
        Write-Host "-> THÔNG TIN: Máy này có sẵn bản quyền NGUYÊN GỐC từ nhà sản xuất."
        Write-Host "Bản quyền này được tích hợp sẵn trong bios của máy."
        
        if ($script:ACT -eq "CHƯA KÍCH HOẠT") {
            [Console]::ForegroundColor = "Yellow"
            Write-Host ""
            Write-Host "LƯU Ý: Do bạn đang chạy phiên bản Windows KHÁC với bản quyền gốc nên máy báo CHƯA KÍCH HOẠT."
            Write-Host "Vui lòng nhập Key hợp lệ, hoặc dùng chức năng [2] ở Menu để khôi phục về đúng bản Windows gốc."
            
            [Console]::ForegroundColor = "White"
            Write-Host ""
            Write-Host "* Bổ sung: Nếu bạn không biết máy tích hợp sẵn bản quyền loại nào, hãy cài lại Windows,"
            Write-Host "  hệ thống sẽ tự nhận diện đúng bản quyền của máy."
        } elseif ($script:HWID_WARN -eq "1") {
            Write-Host "Dù đang dùng Key chung nhưng nhờ có BIOS Key, hệ thống vẫn được ghi nhận nâng cấp hợp lệ."
        } elseif ($script:FULL_PK -ne $script:BIOS_KEY) {
            # NHÁNH BỔ SUNG: Bắt trường hợp Key đang dùng khác với Key BIOS (và không phải Generic Key)
            [Console]::ForegroundColor = "Yellow"
            Write-Host ""
            Write-Host "Tuy nhiên, bạn đang dùng phiên bản Windows KHÁC với bản quyền theo máy."
        }
        Write-Host ""
    }
...
function Show-RestoreOem {
    ...
    Write-Host ""
    [Console]::ForegroundColor = "White"
    Write-Host "[LƯU Ý VỀ PHIÊN BẢN]:"
    Write-Host "- Tính năng hỗ trợ tự động ép chuyển đổi qua lại giữa các bản phiên cao (Pro, Edu, Enterprise,..)"
    Write-Host "- Windows KHÔNG HỖ TRỢ tự động hạ cấp từ bản cao về lại bản Home, Home Single Language."
    Write-Host "  (Nếu Key BIOS của máy là Home/ Home Single Language, bạn bắt buộc phải cài lại Windows)."
    Write-Host ""
    Write-Host "    [1] Tiếp tục"
    Write-Host "    [2] Quay lại"
    Write-Host ""
...
function Go-Buy { Clear-Host; Start-Process "https://madebythanh.art/"; $script:state = "MAIN" }
function Go-Mua { Clear-Host; Start-Process "https://madebythanh.art/"; $script:state = "MAIN" }
...
function Show-ChangeEdition {
    ...
    Write-Host "- Windows Home / Home Single Language / Pro / Enterprise / Education / Pro for Workstations / IoT Enterprise `n "
    ...
    Write-Host "   [5] Windows 10/11 IoT Enterprise"
    ...
    switch ($choice) {
        ...
        "5" { $edition = "IoT Enterprise"; $key = "XQQYW-NFFMW-XJPBH-K8732-CKFFD" }
